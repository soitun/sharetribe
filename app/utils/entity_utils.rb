  ...
  class EntityBuilder
    ...
    def with_result(specs:, data:, on_success:, on_failure:)
      if data.is_a?(ActionController::Parameters)
        Rails.logger.warn("[DEPRECATED] EntityUtils used with ActionController::Parameters. Convert Parameters to Hash using `.to_unsafe_hash` before using EntityUtils")
